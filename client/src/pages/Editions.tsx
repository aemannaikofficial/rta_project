...
function getOrdinalAr(n: number): string {
  return AR_ORDINALS[n] ?? `الإصدار ${n}`;
}
...
export default function Editions() {
  ...
  const { data: dbEditions } = trpc.newsletters.editionsList.useQuery();
      ...
      <section className="bg-[#F5F5F5] py-16">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {dbEditions && dbEditions.length > 0 ? (
              dbEditions.map((ed, i) => {
                ...
                const dateSource = firstNewsletter?.publishDate
                  ? new Date(firstNewsletter.publishDate)
                  : firstNewsletter?.createdAt
                  ? new Date(firstNewsletter.createdAt)
                  : new Date();
                const monthEn = dateSource.toLocaleString("en", { month: "long" });
                const monthAr = dateSource.toLocaleString("ar", { month: "long" });
                      ...
                      <div className="group cursor-pointer bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-100">
                        ...
                        <div className="p-5">
                          <p className={`text-gray-500 text-sm leading-relaxed line-clamp-3 ${isRTL ? "text-right" : ""}`}>
                            {t(forewordEn, forewordAr).slice(0, 180)}...
                          </p>
              ...
              // Fallback: static edition 1 card while DB is loading or empty
              <motion.div
                key="apr-2026"
                initial={{ opacity: 0, y: 30 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                <Link href="/edition/apr-2026">
                  <div className="group cursor-pointer bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-100">
                    <div className="bg-[#003B71] p-6 relative">
                      <div className="absolute top-0 left-0 w-full h-1 bg-[#C8102E]" />
                      <div className="flex items-start justify-between">
                        <div>
                          <span className="text-white/40 text-xs font-medium tracking-wider uppercase">April 2026</span>
                          <h3 className="text-white text-xl font-bold mt-1">{t("AI Newsletter", "نشرة الذكاء الاصطناعي")}</h3>
                        </div>
                        <span className="bg-[#C8102E] text-white text-xs font-bold px-3 py-1 rounded">
                          {t("1st Edition", "الإصدار الأول")}
                        </span>
                      </div>
                    </div>
                    <div className="p-5">
                      <p className={`text-gray-500 text-sm leading-relaxed line-clamp-3 ${isRTL ? "text-right" : ""}`}>
                        {t(
                          "Artificial Intelligence is no longer merely an experimental technology; it has rapidly evolved into a foundational pillar of enterprise transformation and operational excellence...",
                          "لم يعد الذكاء الاصطناعي مجرد تقنية تجريبية؛ بل تطور بسرعة ليصبح ركيزة أساسية للتحول المؤسسي..."
                        )}
                      </p>
                      <div className={`mt-4 flex items-center gap-2 text-[#C8102E] text-sm font-semibold group-hover:gap-3 transition-all ${isRTL ? "flex-row-reverse" : ""}`}>
                        <span>{t("Read Edition", "اقرأ الإصدار")}</span>
