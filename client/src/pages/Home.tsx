        ...
        <div className={`grid grid-cols-1 lg:grid-cols-12 gap-8 items-start`}>
          <div className={`lg:col-span-7 ${isEven ? "" : "lg:order-2"}`}>
            {section.body.map((p, i) => (
              <div 
                key={i} 
                className={`text-gray-600 leading-relaxed mb-4 prose prose-sm max-w-none ${isRTL ? "leading-loose" : ""}`}
                dangerouslySetInnerHTML={{ __html: useBi(p) }} 
              />
            ))}
...
export default function Home() {
  ...
  const staticEdition = getEdition(editionId || "");
  const { isRTL } = useLanguage();
  const [activeSection, setActiveSection] = useState(1);
  // For DB-backed numeric edition IDs (e.g. "/edition/2")
  const isNumericId = editionId ? /^\d+$/.test(editionId) : false;
  const { data: editionsList } = trpc.newsletters.editionsList.useQuery(undefined, { enabled: isNumericId });
  const dbEdition = isNumericId ? editionsList?.find(e => e.editionNumber === parseInt(editionId!)) : undefined;

  let mappedDbEdition: Edition | undefined = undefined;
  if (dbEdition) {
    const firstNl = dbEdition.newsletters[0];
    const dateSource = firstNl?.publishDate ? new Date(firstNl.publishDate) : new Date();
    const monthEn = dateSource.toLocaleString("en", { month: "long" });
    const monthAr = new Intl.DateTimeFormat("ar", { month: "long" }).format(dateSource);
    const year = dateSource.getFullYear();

    mappedDbEdition = {
      id: editionId!,
      number: dbEdition.editionNumber,
      month: { en: monthEn, ar: monthAr },
      year: year,
      title: { en: "AI Newsletter", ar: "نشرة الذكاء الاصطناعي" },
      subtitle: { en: `Edition ${dbEdition.editionNumber}`, ar: `الإصدار ${dbEdition.editionNumber}` },
      foreword: { 
        en: "Welcome to the latest edition of the RTA AI Newsletter. Discover the newest advancements in artificial intelligence shaping the future of mobility.", 
        ar: "مرحباً بكم في الإصدار الأخير من نشرة الذكاء الاصطناعي التابعة لهيئة الطرق والمواصلات. اكتشف أحدث التطورات في الذكاء الاصطناعي التي ترسم مستقبل التنقل." 
      },
      finalNote: { 
      ...
      sections: dbEdition.newsletters.map((n: any, i: number) => ({
        ...
        body: [{ en: n.contentEn || "", ar: n.contentAr || "" }],
        image: n.imageUrl || '',
      })),
      references: [],
    };
  ...
  const edition = staticEdition || mappedDbEdition;
