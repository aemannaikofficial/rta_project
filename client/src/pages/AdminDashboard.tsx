...
export default function AdminDashboard() {
  ...
  const editionsQuery = trpc.newsletters.editionsList.useQuery();

  const totalEditions = editionsQuery.data?.length ?? 0;
  const totalUsers = usersQuery.data?.length ?? 0;
  const totalSections = editionsQuery.data?.reduce((sum, e) => sum + e.newsletters.length, 0) ?? 0;
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
          {editionsQuery.isLoading ? (
            <p className="text-gray-400 text-sm">Loading editions...</p>
          ) : editionsQuery.data && editionsQuery.data.length > 0 ? (
            editionsQuery.data.map((ed) => {
              const firstN = ed.newsletters[0];
              const label = (() => {
                const s = ["th", "st", "nd", "rd"];
                const v = ed.editionNumber % 100;
                return ed.editionNumber + (s[(v - 20) % 10] || s[v] || s[0]) + " Edition";
              })();
              return (
                <div key={ed.editionNumber} className="flex items-center gap-4 p-4 rounded-lg border border-gray-100 hover:border-[#C8102E]/30 transition-colors">
                  <div className="w-12 h-12 rounded-lg bg-[#003B71] flex items-center justify-center flex-shrink-0">
                    <Newspaper className="w-5 h-5 text-white" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-[#003B71] text-sm">{firstN?.titleEn || "AI Newsletter"} — {label}</p>
                    <p className="text-xs text-gray-400 mt-0.5">{ed.newsletters.length} newsletter{ed.newsletters.length !== 1 ? "s" : ""}</p>
                  </div>
            ...
            <p className="text-gray-400 text-sm">No editions yet.</p>
          )}
        </div>
      </div>
    </div>
